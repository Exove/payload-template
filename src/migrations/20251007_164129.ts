...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE INDEX \`media_sizes_admin_thumbnail_sizes_admin_thumbnail_filena_idx\` ON \`media\` (\`sizes_admin_thumbnail_filename\`);`)
  ...
  await db.run(sql`CREATE TABLE \`articles_blocks_tabs_tabs\` (
  	\`_order\` integer NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`articles_blocks_tabs_tabs_order_idx\` ON \`articles_blocks_tabs_tabs\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`articles_blocks_tabs_tabs_parent_id_idx\` ON \`articles_blocks_tabs_tabs\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`articles_blocks_tabs\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`_path\` text NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`articles_blocks_tabs_order_idx\` ON \`articles_blocks_tabs\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`articles_blocks_tabs_parent_id_idx\` ON \`articles_blocks_tabs\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`articles_blocks_tabs_path_idx\` ON \`articles_blocks_tabs\` (\`_path\`);`)
  ...
  await db.run(sql`CREATE TABLE \`_articles_v_blocks_tabs_tabs\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`label\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`_articles_v_blocks_tabs_tabs_order_idx\` ON \`_articles_v_blocks_tabs_tabs\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`_articles_v_blocks_tabs_tabs_parent_id_idx\` ON \`_articles_v_blocks_tabs_tabs\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`_articles_v_blocks_tabs\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`_path\` text NOT NULL,
  	\`id\` integer PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`_articles_v_blocks_tabs_order_idx\` ON \`_articles_v_blocks_tabs\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`_articles_v_blocks_tabs_parent_id_idx\` ON \`_articles_v_blocks_tabs\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`_articles_v_blocks_tabs_path_idx\` ON \`_articles_v_blocks_tabs\` (\`_path\`);`)
  ...
  await db.run(sql`CREATE TABLE \`front_page_hero\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`_locale\` text NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`front_page_hero_order_idx\` ON \`front_page_hero\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`front_page_hero_parent_id_idx\` ON \`front_page_hero\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`front_page_hero_locale_idx\` ON \`front_page_hero\` (\`_locale\`);`)
  await db.run(sql`CREATE INDEX \`front_page_hero_image_idx\` ON \`front_page_hero\` (\`image_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`front_page_blocks_carousel_items\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` text NOT NULL,
  	\`_locale\` text NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`front_page_blocks_carousel_items_order_idx\` ON \`front_page_blocks_carousel_items\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`front_page_blocks_carousel_items_parent_id_idx\` ON \`front_page_blocks_carousel_items\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`front_page_blocks_carousel_items_locale_idx\` ON \`front_page_blocks_carousel_items\` (\`_locale\`);`)
  await db.run(sql`CREATE INDEX \`front_page_blocks_carousel_items_image_idx\` ON \`front_page_blocks_carousel_items\` (\`image_id\`);`)
  await db.run(sql`CREATE TABLE \`front_page_blocks_carousel\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`_path\` text NOT NULL,
  	\`_locale\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`front_page_blocks_carousel_order_idx\` ON \`front_page_blocks_carousel\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`front_page_blocks_carousel_parent_id_idx\` ON \`front_page_blocks_carousel\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`front_page_blocks_carousel_path_idx\` ON \`front_page_blocks_carousel\` (\`_path\`);`)
  await db.run(sql`CREATE INDEX \`front_page_blocks_carousel_locale_idx\` ON \`front_page_blocks_carousel\` (\`_locale\`);`)
  ...
  await db.run(sql`CREATE TABLE \`front_page_blocks_rich_text\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`_path\` text NOT NULL,
  	\`_locale\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`front_page_blocks_rich_text_order_idx\` ON \`front_page_blocks_rich_text\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`front_page_blocks_rich_text_parent_id_idx\` ON \`front_page_blocks_rich_text\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`front_page_blocks_rich_text_path_idx\` ON \`front_page_blocks_rich_text\` (\`_path\`);`)
  await db.run(sql`CREATE INDEX \`front_page_blocks_rich_text_locale_idx\` ON \`front_page_blocks_rich_text\` (\`_locale\`);`)
  ...
  `)
  await db.run(sql`CREATE INDEX \`front_page__status_idx\` ON \`front_page\` (\`_status\`);`)
  ...
  await db.run(sql`CREATE TABLE \`_front_page_v_version_hero\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`_locale\` text NOT NULL,
  	\`id\` integer PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`_front_page_v_version_hero_order_idx\` ON \`_front_page_v_version_hero\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_version_hero_parent_id_idx\` ON \`_front_page_v_version_hero\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_version_hero_locale_idx\` ON \`_front_page_v_version_hero\` (\`_locale\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_version_hero_image_idx\` ON \`_front_page_v_version_hero\` (\`image_id\`);`)
  await db.run(sql`CREATE TABLE \`_front_page_v_blocks_large_featured_post\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`_path\` text NOT NULL,
  	\`_locale\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_large_featured_post_order_idx\` ON \`_front_page_v_blocks_large_featured_post\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_large_featured_post_parent_id_idx\` ON \`_front_page_v_blocks_large_featured_post\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_large_featured_post_path_idx\` ON \`_front_page_v_blocks_large_featured_post\` (\`_path\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_large_featured_post_locale_idx\` ON \`_front_page_v_blocks_large_featured_post\` (\`_locale\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_large_featured_post_image_idx\` ON \`_front_page_v_blocks_large_featured_post\` (\`image_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`_front_page_v_blocks_small_featured_posts_wrapper\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`_path\` text NOT NULL,
  	\`_locale\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_small_featured_posts_wrapper_order_idx\` ON \`_front_page_v_blocks_small_featured_posts_wrapper\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_small_featured_posts_wrapper_parent_id_idx\` ON \`_front_page_v_blocks_small_featured_posts_wrapper\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_small_featured_posts_wrapper_path_idx\` ON \`_front_page_v_blocks_small_featured_posts_wrapper\` (\`_path\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_small_featured_posts_wrapper_locale_idx\` ON \`_front_page_v_blocks_small_featured_posts_wrapper\` (\`_locale\`);`)
  await db.run(sql`CREATE TABLE \`_front_page_v_blocks_carousel_items\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`_locale\` text NOT NULL,
  	\`id\` integer PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_carousel_items_order_idx\` ON \`_front_page_v_blocks_carousel_items\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_carousel_items_parent_id_idx\` ON \`_front_page_v_blocks_carousel_items\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_carousel_items_locale_idx\` ON \`_front_page_v_blocks_carousel_items\` (\`_locale\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_carousel_items_image_idx\` ON \`_front_page_v_blocks_carousel_items\` (\`image_id\`);`)
  await db.run(sql`CREATE TABLE \`_front_page_v_blocks_carousel\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`_path\` text NOT NULL,
  	\`_locale\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_carousel_order_idx\` ON \`_front_page_v_blocks_carousel\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_carousel_parent_id_idx\` ON \`_front_page_v_blocks_carousel\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_carousel_path_idx\` ON \`_front_page_v_blocks_carousel\` (\`_path\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_carousel_locale_idx\` ON \`_front_page_v_blocks_carousel\` (\`_locale\`);`)
  await db.run(sql`CREATE TABLE \`_front_page_v_blocks_link_list_links\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`_locale\` text NOT NULL,
  	\`id\` integer PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_link_list_links_order_idx\` ON \`_front_page_v_blocks_link_list_links\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_link_list_links_parent_id_idx\` ON \`_front_page_v_blocks_link_list_links\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_link_list_links_locale_idx\` ON \`_front_page_v_blocks_link_list_links\` (\`_locale\`);`)
  await db.run(sql`CREATE TABLE \`_front_page_v_blocks_link_list\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`_path\` text NOT NULL,
  	\`_locale\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_link_list_order_idx\` ON \`_front_page_v_blocks_link_list\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_link_list_parent_id_idx\` ON \`_front_page_v_blocks_link_list\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_link_list_path_idx\` ON \`_front_page_v_blocks_link_list\` (\`_path\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_link_list_locale_idx\` ON \`_front_page_v_blocks_link_list\` (\`_locale\`);`)
  await db.run(sql`CREATE TABLE \`_front_page_v_blocks_contacts\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`_path\` text NOT NULL,
  	\`_locale\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_contacts_order_idx\` ON \`_front_page_v_blocks_contacts\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_contacts_parent_id_idx\` ON \`_front_page_v_blocks_contacts\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_contacts_path_idx\` ON \`_front_page_v_blocks_contacts\` (\`_path\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_contacts_locale_idx\` ON \`_front_page_v_blocks_contacts\` (\`_locale\`);`)
  await db.run(sql`CREATE TABLE \`_front_page_v_blocks_quote\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`_path\` text NOT NULL,
  	\`_locale\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_quote_order_idx\` ON \`_front_page_v_blocks_quote\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_quote_parent_id_idx\` ON \`_front_page_v_blocks_quote\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_quote_path_idx\` ON \`_front_page_v_blocks_quote\` (\`_path\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_quote_locale_idx\` ON \`_front_page_v_blocks_quote\` (\`_locale\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_quote_image_idx\` ON \`_front_page_v_blocks_quote\` (\`image_id\`);`)
  await db.run(sql`CREATE TABLE \`_front_page_v_blocks_rich_text\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`_path\` text NOT NULL,
  	\`_locale\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_rich_text_order_idx\` ON \`_front_page_v_blocks_rich_text\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_rich_text_parent_id_idx\` ON \`_front_page_v_blocks_rich_text\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_rich_text_path_idx\` ON \`_front_page_v_blocks_rich_text\` (\`_path\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_rich_text_locale_idx\` ON \`_front_page_v_blocks_rich_text\` (\`_locale\`);`)
  await db.run(sql`CREATE TABLE \`_front_page_v_blocks_dynamic_list_collections\` (
  	\`order\` integer NOT NULL,
  	\`parent_id\` integer NOT NULL,
  	\`value\` text,
  	\`locale\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_dynamic_list_collections_order_idx\` ON \`_front_page_v_blocks_dynamic_list_collections\` (\`order\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_dynamic_list_collections_parent_idx\` ON \`_front_page_v_blocks_dynamic_list_collections\` (\`parent_id\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_dynamic_list_collections_locale_idx\` ON \`_front_page_v_blocks_dynamic_list_collections\` (\`locale\`);`)
  await db.run(sql`CREATE TABLE \`_front_page_v_blocks_dynamic_list_items\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`_locale\` text NOT NULL,
  	\`id\` integer PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_dynamic_list_items_order_idx\` ON \`_front_page_v_blocks_dynamic_list_items\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_dynamic_list_items_parent_id_idx\` ON \`_front_page_v_blocks_dynamic_list_items\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_dynamic_list_items_locale_idx\` ON \`_front_page_v_blocks_dynamic_list_items\` (\`_locale\`);`)
  await db.run(sql`CREATE TABLE \`_front_page_v_blocks_dynamic_list\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`_path\` text NOT NULL,
  	\`_locale\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_dynamic_list_order_idx\` ON \`_front_page_v_blocks_dynamic_list\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_dynamic_list_parent_id_idx\` ON \`_front_page_v_blocks_dynamic_list\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_dynamic_list_path_idx\` ON \`_front_page_v_blocks_dynamic_list\` (\`_path\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_blocks_dynamic_list_locale_idx\` ON \`_front_page_v_blocks_dynamic_list\` (\`_locale\`);`)
  await db.run(sql`CREATE TABLE \`_front_page_v\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`version__status\` text DEFAULT 'draft',
  	\`version_updated_at\` text,
  	\`version_created_at\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`_front_page_v_version_version__status_idx\` ON \`_front_page_v\` (\`version__status\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_created_at_idx\` ON \`_front_page_v\` (\`created_at\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_updated_at_idx\` ON \`_front_page_v\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_snapshot_idx\` ON \`_front_page_v\` (\`snapshot\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_published_locale_idx\` ON \`_front_page_v\` (\`published_locale\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_latest_idx\` ON \`_front_page_v\` (\`latest\`);`)
  await db.run(sql`CREATE TABLE \`_front_page_v_locales\` (
  	\`version_meta_title\` text,
  	\`version_meta_description\` text,
  	\`version_meta_image_id\` integer,
  	\`id\` integer PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`_front_page_v_version_meta_version_meta_image_idx\` ON \`_front_page_v_locales\` (\`version_meta_image_id\`,\`_locale\`);`)
  await db.run(sql`CREATE UNIQUE INDEX \`_front_page_v_locales_locale_parent_id_unique\` ON \`_front_page_v_locales\` (\`_locale\`,\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`_front_page_v_rels\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`order\` integer,
  	\`parent_id\` integer NOT NULL,
  	\`path\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`_front_page_v_rels_order_idx\` ON \`_front_page_v_rels\` (\`order\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_rels_parent_idx\` ON \`_front_page_v_rels\` (\`parent_id\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_rels_path_idx\` ON \`_front_page_v_rels\` (\`path\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_rels_locale_idx\` ON \`_front_page_v_rels\` (\`locale\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_rels_articles_id_idx\` ON \`_front_page_v_rels\` (\`articles_id\`,\`locale\`);`)
  await db.run(sql`CREATE INDEX \`_front_page_v_rels_contacts_id_idx\` ON \`_front_page_v_rels\` (\`contacts_id\`,\`locale\`);`)
